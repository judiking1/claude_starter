...
코드를 한 줄도 모르는 사람이 와도, 설치하고 `/starter my-project` 한 번이면 준비 끝. 12개 슬래시 커맨드가 프로젝트 기획부터 배포까지 전 과정을 자동화합니다.
...
```
You:    /starter roller-coaster-tycoon
...
### 2. 프로젝트 템플릿 (`/starter`로 프로젝트마다 복사)
...
| **프로젝트 템플릿** | 각 프로젝트 폴더 | 프로젝트마다 `/starter` | 코딩 규칙, 문서 |

```
~/.claude/skills/claude_starter/     각 프로젝트/
├── starter/SKILL.md      (/starter) ├── CLAUDE.md          ← Claude가 매 세션마다 읽음
...
| 스킬 | 역할 | 하는 일 |
|------|------|--------|
| `/starter` | **프로젝트 초기화** | 현재 폴더에 CLAUDE.md + rules 7개 + docs를 자동 복사. 프로젝트 이름 반영. 이후 어떤 프로젝트에서든 동일한 코딩 규칙이 적용됨 |
...
```
/starter  →  /project-kickoff  →  /design-system init  →  /plan-review
                                                            │
                                                            ▼
                                                     코딩 (Claude와 대화)
                                                     큰 기능은 /team-lead
...
> **다른 스택을 쓰고 싶으면?** `/project-kickoff`에서 대화하며 바꿀 수 있습니다. 또는 `/starter` 후 `CLAUDE.md`와 `.claude/rules/`를 수동으로 수정하세요.
...
`./setup` 스크립트가 `claude_starter/` 안의 스킬을 `~/.claude/skills/스킬명/`으로 자동 복사합니다. 설치 후 Claude Code를 열면 12개 스킬이 바로 사용 가능합니다.
...
│
├── starter/SKILL.md           # /starter — 프로젝트 템플릿 초기화
...
│
├── template/                  # /starter가 여기서 읽어서 프로젝트에 복사
...
```bash
cd ~/.claude/skills/claude_starter && git pull && ./setup
```

새 스킬이 추가되거나 기존 스킬이 개선되면 `git pull && ./setup`으로 바로 반영됩니다.
...
`/starter` 후 `CLAUDE.md`의 기술 스택 섹션과 `.claude/rules/`를 수정하세요. `/project-kickoff`에서 대화하며 바꿀 수도 있습니다.
